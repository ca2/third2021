...
extern bool get_window_exe(struct dstr *name, HWND window, bool * pbExeIsLocked);
...
void dc_capture_capture(struct ca2plugin * wc, struct dc_capture *capture, HWND window)
{
   ...
   if (wc->m_hmutex != NULL)
   {
      ...
      if (WaitForSingleObject(wc->m_hmutex, 1) == WAIT_OBJECT_0)
      {
...
bool get_window_exe(struct dstr *name, HWND window, bool * pbExeIsLocked)
{
...
static void add_window(obs_property_t *p, HWND hwnd, add_window_cb callback)
{
   ...
   if (!get_window_exe(&exe, hwnd, NULL))
      return;
...
static int ca2_window(HWND window)
{
   ...
   if (dstr_ncmpi(&cur_class, "windows_interaction_impl", 14) == 0)
   {
                         ...
                         const char *exe)
{
   ...
   if (!get_window_exe(&cur_exe, window, NULL))
      return 0;
   get_window_title(&cur_title, window);
   get_window_class(&cur_class, window);
                 ...
                 const char *exe)
{
   ...
   while (window)
   {
      ...
      bool bExeLocked = false;
      struct dstr cur_exe = { 0 };
      if (get_window_exe(&cur_exe, window, &bExeLocked) && ca2_window(window))
      {
         dstr_free(&cur_exe);
