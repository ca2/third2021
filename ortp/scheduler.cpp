...
extern void rtp_session_process (RtpSession * session, u32 time, RtpScheduler *sched);
...
void rtp_scheduler_init(RtpScheduler *sched)
{
   ...
   ortp_mutex_init(&sched->lock,nullptr);
   ortp_cond_init(&sched->unblock_select_cond,nullptr);
...
RtpScheduler * rtp_scheduler_new()
{
   ...
   __memset(sched,0,sizeof(RtpScheduler));
...
void rtp_scheduler_set_timer(RtpScheduler *sched,RtpTimer *timer)
{
   ...
   sched->e_timer_inc=(u32) ((timer->interval.tv_usec/1000) + (timer->interval.tv_sec*1000));
}
...
void rtp_scheduler_start(RtpScheduler *sched)
{
   if (sched->thread_running==0){
      ...
      ortp_single_lock(&sched->lock);
      ortp_thread_create(&sched->thread, nullptr, rtp_scheduler_schedule,(void*)sched);
...
void rtp_scheduler_stop(RtpScheduler *sched)
{
   if (sched->thread_running==1)
   {
      ...
      ortp_thread_join(sched->thread, nullptr);
   }
...
void * rtp_scheduler_schedule(void * psched)
{
   ...
   while(sched->thread_running)
   {
      ...
      result mask and see if they have to leave, or to wait for next millis*/
      //ortp_message("scheduler: sleeping.");
      timer->e_timer_do();
      sched->time_+=sched->e_timer_inc;
   }
   /* when leaving the thread, stop the timer */
   timer->e_timer_uninit();
   return nullptr;
}
...
void rtp_scheduler_remove_session(RtpScheduler *sched, RtpSession *session)
{
   RtpSession *tmp;
   i32 cond=1;
   return_if_fail(session!=nullptr);
   if (!(session->flags & RTP_SESSION_IN_SCHEDULER)){
