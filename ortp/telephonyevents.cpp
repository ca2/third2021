...
**/
i32 rtp_session_telephone_events_supported(RtpSession *session)
{
   ...
   /*debug_print("Telephone event payloadtype is %i\n",session->telephone_events_pt);*/
 ...
 * @return the payload type number used for telephony events if found, -1 if not found.
**/
i32 rtp_session_send_telephone_events_supported(RtpSession *session)
{
   /* search for a telephony event payload in the current profile */
   session->snd.telephone_events_pt=rtp_profile_get_payload_number_from_mime(session->snd.profile,"telephone-event");
   /*debug_print("Telephone event payloadtype is %i\n",session->telephone_events_pt);*/
 ...
 *
 * @return the payload type number used for telephony events if found, -1 if not found.
**/i32 rtp_session_recv_telephone_events_supported(RtpSession *session)
{
   /* search for a telephony event payload in the current profile */
   session->rcv.telephone_events_pt=rtp_profile_get_payload_number_from_mime(session->rcv.profile,"telephone-event");
   /*debug_print("Telephone event payloadtype is %i\n",session->telephone_events_pt);*/
 ...
 *
 * @return a message block containing the rtp packet if successfull, nullptr if the rtp session
 *cannot support telephony event (because the rtp profile it is bound to does not include
 *a telephony event payload type).
**/
mblk_t   *rtp_session_create_telephone_event_packet(RtpSession *session, i32 start)
{
   mblk_t *mp;
   rtp_header_t *rtp;
   
   return_val_if_fail(session->snd.telephone_events_pt!=-1,nullptr);
   
   mp=allocb(RTP_FIXED_HEADER_SIZE+TELEPHONY_EVENTS_ALLOCATED_SIZE,BPRI_MED);
   if (mp==nullptr) return nullptr;
 ...
 *@return 0 on success.
**/
i32 rtp_session_add_telephone_event(RtpSession *session,
         mblk_t *packet, u8 event, i32 end, u8 volume, u16 duration)
{
...
**/
i32 rtp_session_send_dtmf(RtpSession *session, char dtmf, u32 userts)
{
 ...
 * @return 0 if successfull, -1 if the session cannot support telephony events or if the dtmf given as argument is not valid.
**/
i32 rtp_session_send_dtmf2(RtpSession *session, char dtmf, u32 userts, i32 duration)
{
   mblk_t *m1,*m2,*m3;
   i32 tev_type;
   i32 durationtier = duration/3;
...
static void notify_events_ended(RtpSession *session, telephone_event_t *events, i32 num){
   i32 i;
   for (i=0;i<num;i++){
      if (events[i].E==1){
...
void rtp_session_check_telephone_events(RtpSession *session, mblk_t *m0)
{
   ...
   if (hdr->markbit==1)
   {
      ...
      if (session->current_tev!=nullptr) {
         freemsg(session->current_tev);
         session->current_tev=nullptr;
      }
      session->current_tev=copymsg(m0);
      /* handle the case where the events are i16 enough to end within the packet that has the marker bit*/
