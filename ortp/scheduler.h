...
struct _RtpScheduler {
   ...
   i32      all_max;      /* the highest pos in the all mask */
   ...
   i32      e_max;
   i32 max_sessions;      /* the number of position in the masks */
  /* GMutex  *unblock_select_mutex; */
   ...
   u32 time_;       /*number of miliseconds elapsed since the start of the thread */
   u32 e_timer_inc;   /* the timer increment in milisec */
};
...
#define rtp_scheduler_lock(sched)   ortp_single_lock(&(sched)->lock)
