...
{
        u32 ntp_timestamp_msw;
        u32 ntp_timestamp_lsw;
        u32 rtp_timestamp;
        u32 senders_packet_count;
        u32 senders_octet_count;
} sender_info_t;

u64 sender_info_get_ntp_timestamp(const sender_info_t *si);
...
{
        u32 ssrc;
        u32 fl_cnpl;/*fraction lost + cumulative number of packet lost*/
        u32 ext_high_seq_num_rec; /*extended highest sequence number received */
        u32 interarrival_jitter;
        u32 lsr; /*last SR */
        u32 delay_snc_last_sr; /*delay since last sr*/
} report_block_t;

#define report_block_get_ssrc(rb) \
   ntohl((rb)->ssrc)
#define report_block_get_fraction_lost(rb) \
   (((u32)ntohl((rb)->fl_cnpl))>>24)
#define report_block_get_cum_packet_loss(rb) \
   (((u32)ntohl((rb)->fl_cnpl)) & 0xFFFFFF)
...
#define sdes_chunk_get_csrc(ca)   ntohl((ca)->csrc)
...
typedef struct rtcp_bye
{
   ...
   u32 ssrc[1];  /* the bye may contain several ssrc/csrc */
} rtcp_bye_t;
...
typedef void (*SdesItemFoundCallback)(void *user_data, u32 csrc, rtcp_sdes_type_t t, const char *content, u8 content_len); 
