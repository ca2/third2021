...
extern mblk_t *rtcp_create_simple_bye_packet(u32 ssrc, const char *reason);
extern i32 rtcp_sr_init(RtpSession *session, char *buf, i32 size);
extern i32 rtcp_rr_init(RtpSession *session, char *buf, i32 size);



/* this function initialize all session parameter's that depend on the payload type */
static void payload_type_changed(RtpSession *session, PayloadType *payloadtype){
   jitter_control_set_payload(&session->rtp.jittctl, payloadtype);
   session->rtp.rtcp_report_snt_interval=RTCP_DEFAULT_REPORT_INTERVAL*payloadtype->clock_rate;
   rtp_session_set_time_jump_limit(session,session->rtp.time_jump);
   if (payloadtype->type==PAYLOAD_VIDEO){
...
void wait_pt_init(WaitPoint *wp){
   ortp_mutex_init(&wp->lock,nullptr);
   ortp_cond_init(&wp->cond,nullptr);
   wp->time=0;
   wp->wakeup=FALSE;
}
void wait_pt_uninit(WaitPoint *wp){
   ortp_cond_destroy(&wp->cond);
   ortp_mutex_destroy(&wp->lock);
}

#define wait_pt_lock(wp) ortp_single_lock(&(wp)->lock)
#define wait_pt_unlock(wp) ortp_mutex_unlock(&(wp)->lock)

void wait_pt_wakeup_at(WaitPoint *wp, u32 t, bool_t dosleep){
   wp->time=t;
   wp->wakeup=TRUE;
...
bool_t wait_pt_check(WaitPoint *wp, u32 t){
   bool_t ok=FALSE;
...
#define wait_pt_wakeup(wp) ortp_cond_signal(&(wp)->cond);

extern void rtp_parse(RtpSession *session, mblk_t *mp, u32 local_str_ts,
      struct sockaddr *addr, socklen_t addrlen);


static u32 u32_random(){
   return (u32) random();
}


#define RTP_SEQ_IS_GREATER(seq1,seq2)\
   ((u16)((u16)(seq1) - (u16)(seq2))< (u16)(1<<15))
...
mblk_t *rtp_getq(queue_t *q,u32 timestamp, i32 *rejected)
{
   mblk_t *tmp,*ret=nullptr,*old=nullptr;
   rtp_header_t *tmprtp;
   u32 ts_found=0;
...
mblk_t *rtp_getq_permissive(queue_t *q,u32 timestamp, i32 *rejected)
{
   mblk_t *tmp,*ret=nullptr;
   rtp_header_t *tmprtp;
...
void
rtp_session_init (RtpSession * session, i32 mode)
{
   JBParameters jbp;
   __memset (session, 0, sizeof (RtpSession));
   ...
   if ((mode == RTP_SESSION_SENDONLY) || (mode == RTP_SESSION_SENDRECV))
   {
      ...
      session->snd.ssrc=u32_random();
      /* set default source description */
      rtp_session_set_source_description(session,"unknown@unknown",nullptr,nullptr,
            nullptr,nullptr,"oRTP-" ORTP_VERSION,"This is free sofware (LGPL) !");
   }
   ...
   session->rtp.socket= (ortp_socket_t)-1;
   session->rtcp.socket=(ortp_socket_t)-1;
#ifndef WIN32
   session->rtp.snd_socket_size=0;   /*use OS default value unless on windows where they are definitely too i16*/
   ...
   wait_pt_init(&session->snd.wp);
   wait_pt_init(&session->rcv.wp);
...
**/
void
rtp_session_set_scheduling_mode (RtpSession * session, i32 yesno)
{
   if (yesno)
   {
      RtpScheduler *sched;
...
**/
void
rtp_session_set_blocking_mode (RtpSession * session, i32 yesno)
{
   if (yesno){
 ...
 *   a ::account::user supplied function in charge of processing it. The application can register
 ...
 *                  a const char * explaining the error, an i32 errno error code and the user_data as usual.
 *
 ...
 * @param user_data   a pter to any data to be passed when invoking the callback.
 *
**/
i32
rtp_session_signal_connect (RtpSession * session, const char *signal_name,
             RtpCallback cb, uptr user_data)
{
   OList *elem;
   for (elem=session->signal_tables;elem!=nullptr;elem=o_list_next(elem)){
 ...
 *
 * @param session a rtp session.
 * @param ssrc an u32 32bit integer representing the synchronisation source identifier (SSRC).
**/
void
rtp_session_set_ssrc (RtpSession * session, u32 ssrc)
{
   session->snd.ssrc = ssrc;
}


void rtp_session_update_payload_type(RtpSession *session, i32 paytype){
   /* check if we support this payload type */
   PayloadType *payloadtype=rtp_profile_get_payload(session->rcv.profile,paytype);
   session->hw_recv_pt=paytype;
   if (payloadtype!=0){
      ortp_message ("payload type changed to %i(%s) !",
             paytype,payloadtype->mime_type);
      payload_type_changed(session, payloadtype);
   }else{
...
i32
rtp_session_set_send_payload_type (RtpSession * session, i32 paytype)
{
   session->snd.payloadtype=paytype;
   return 0;
}
...
i32 rtp_session_get_send_payload_type(const RtpSession *session){
   return session->snd.payloadtype;
}
...
i32
rtp_session_set_recv_payload_type (RtpSession * session, i32 paytype)
{
   PayloadType *payloadtype;
   session->rcv.payloadtype=paytype;
   session->hw_recv_pt=paytype;
   payloadtype=rtp_profile_get_payload(session->rcv.profile,paytype);
...
i32 rtp_session_get_recv_payload_type(const RtpSession *session){
   return session->rcv.payloadtype;
}
 ...
 * @return 0 on success, -1 if the payload is not defined.
**/
i32 rtp_session_set_payload_type(RtpSession *session, i32 payloadtype){
   if (rtp_session_set_send_payload_type(session, payloadtype)<0) return -1;
   if (rtp_session_set_recv_payload_type(session, payloadtype)<0) return -1;
   return 0;
}
...
static void rtp_header_init_from_session(rtp_header_t *rtp, RtpSession *session){
   ...
   rtp->markbit= 0;
   rtp->cc = 0;
   rtp->paytype = session->snd.payloadtype;
 ...
 *   If payload_size is zero, thus an is_empty packet (just a RTP header) is returned.
 *
 *@param session a rtp session.
...
{
   mblk_t *mp;
   i32 msglen=header_size+payload_size;
   rtp_header_t *rtp;
   ...
   if (payload_size){
      ::memcpy_dup(mp->b_wptr,payload,payload_size);
 ...
 *   packet concatenation involving copies to be done in ::account::user-space.
 *  @freefn can be nullptr, in that case payload will be kept untouched.
 *
 * @param session a rtp session.
...
{
   mblk_t *mp,*mpayload;
   i32 header_size=RTP_FIXED_HEADER_SIZE; /* revisit when support for csrc is done */
   rtp_header_t *rtp;
   ...
   rtp_header_init_from_session(rtp,session);
   mp->b_wptr+=header_size;
   /* create a mblk_t around the ::account::user supplied payload buffer */
...
i32
__rtp_session_sendm_with_ts (RtpSession * session, mblk_t *mp, u32 packet_ts, u32 send_ts)
{
   rtp_header_t *rtp;
   u32 packet_time;
   i32 error = 0;
   ...
   if (session->flags & RTP_SESSION_SEND_NOT_STARTED)
   {
      ...
      {
      gettimeofday(&session->last_recv_time, nullptr);
      }
   ...
   if (session->flags & RTP_SESSION_SCHEDULED)
   {
      ...
      {
         wait_pt_wakeup_at(&session->snd.wp,packet_time,(session->flags & RTP_SESSION_BLOCKING_MODE)!=0);
      ...
      wait_pt_unlock(&session->snd.wp);
   }

   if(mp==nullptr) {/*for people who just want to be blocked but
...
i32 rtp_session_sendm_with_ts(RtpSession *session, mblk_t *packet, u32 timestamp){
...
**/
i32
rtp_session_send_with_ts (RtpSession * session, const u8 * buffer, i32 len,
           u32 userts)
{
   mblk_t *m;
   i32 err;
#ifdef USE_SENDMSG
   m=rtp_session_create_packet_with_data(session,(u8*)buffer,len,nullptr);
#else
   m = rtp_session_create_packet(session,RTP_FIXED_HEADER_SIZE,(u8*)buffer,len);
#endif
...
static void payload_type_changed_notify(RtpSession *session, i32 paytype){
   session->rcv.payloadtype = paytype;
 ...
 *   and the ::account::user timestamp given here.
 *
...
mblk_t *
rtp_session_recvm_with_ts (RtpSession * session, u32 user_ts)
{
   mblk_t *mp = nullptr;
   rtp_header_t *rtp;
   u32 ts;
   ...
   if (mp!=nullptr){
      i32 msgsize=msgdsize(mp);
      ortp_global_stats.recv += msgsize;
      stream->stats.recv += msgsize;
      rtp_signal_table_emit2(&session->on_telephone_event_packet,(uptr)mp);
   ...
   /*calculate the stream timestamp from the ::account::user timestamp */
   ...
   if (mp != nullptr)
   {
      i32 msgsize = msgdsize (mp);   /* evaluate how much bytes (including header) is received by app */
      u32 packet_ts;
      ortp_global_stats.recv += msgsize;
      stream->stats.recv += msgsize;
      rtp = (rtp_header_t *) mp->b_rptr;
      ...
      if (session->rcv.payloadtype != rtp->paytype)
      {
 ...
 *   where the ::account::user supplied buffer @buffer is not large enough to get all the data
 ...
 *   of this packet is then copied into the ::account::user supplied buffer in an intelligent manner:
 ...
 *   then the number of bytes written in the ::account::user supplied buffer is returned. If no packets
 ...
 *@param session a rtp session.
 *@param buffer a ::account::user supplied buffer to write the data.
 *@param len the length in bytes of the ::account::user supplied buffer.
 ...
 *
**/
i32 rtp_session_recv_with_ts (RtpSession * session, u8 * buffer,
                i32 len, u32 ts, i32 * have_more){
   mblk_t *mp=nullptr;
   i32 plen,blen=0;
   *have_more=0;
   while(1){
      if (session->pending){
         ...
         if (mp!=nullptr) rtp_get_payload(mp,&mp->b_rptr);
      }
      if (mp){
         plen=(i32) (mp->b_wptr-mp->b_rptr);
         if (plen<=len){
            ::memcpy_dup(buffer,mp->b_rptr,plen);
            buffer+=plen;
            blen+=plen;
            len-=plen;
            freemsg(mp);
            ...
            ::memcpy_dup(buffer,mp->b_rptr,len);
            mp->b_rptr+=len;
            buffer+=len;
            blen+=len;
            len=0;
...
u32 rtp_session_get_current_send_ts(RtpSession *session)
{
   ...
   RtpScheduler *sched=session->sched;
   PayloadType *payload;
   payload=rtp_profile_get_payload(session->snd.profile,session->snd.payloadtype);
   return_val_if_fail(payload!=nullptr, 0);
   ...
   userts=  (u32)( ( (double)(session_time) * (double) payload->clock_rate )/ 1000.0)
            + session->rtp.snd_ts_offset;
   return userts;
}
...
u32 rtp_session_get_current_recv_ts(RtpSession *session){
   ...
   PayloadType *payload;
   payload=rtp_profile_get_payload(session->rcv.profile,session->rcv.payloadtype);
   return_val_if_fail(payload!=nullptr, 0);
   if ( (session->flags & RTP_SESSION_SCHEDULED)==0 ){
      ortp_warning("can't guess current timestamp because session is not scheduled.");
      return 0;
 ...
 *
**/
void rtp_session_set_time_jump_limit(RtpSession *session, i32 milisecs){
   u32 ts;
   session->rtp.time_jump=milisecs;
   ts=rtp_session_time_to_ts(session,milisecs);
   if (ts==0)
      session->rtp.ts_jump = (u32) 1<<31;   /* do not detect ts jump */
...
void rtp_session_release_sockets(RtpSession *session){
   ...
   session->rtp.socket=(ortp_socket_t)-1;
   session->rtcp.socket=(ortp_socket_t)-1;
   if (session->rtp.tr!=nullptr)
     ortp_free(session->rtp.tr);
   if (session->rtcp.tr!=nullptr)
...
void rtp_session_dispatch_event(RtpSession *session, OrtpEvent *ev){
   OList *it;
   i32 i;
   for(i=0,it=session->eventqs;it!=nullptr;it=it->next,++i){
...
void rtp_session_uninit (RtpSession * session)
{
   ...
   if (session->eventqs!=nullptr) o_list_free(session->eventqs);
   /* close sockets */
   rtp_session_release_sockets(session);

   wait_pt_uninit(&session->snd.wp);
   wait_pt_uninit(&session->rcv.wp);
   if (session->current_tev!=nullptr) freemsg(session->current_tev);
   if (session->rtp.cached_mp!=nullptr) freemsg(session->rtp.cached_mp);
   if (session->rtcp.cached_mp!=nullptr) freemsg(session->rtcp.cached_mp);
...
void rtp_session_resync(RtpSession *session){
   ...
   jitter_control_init(&session->rtp.jittctl,-1,nullptr);
}
...
void rtp_session_reset_stats(RtpSession *session){
   __memset(&session->rtp.stats,0,sizeof(rtp_stats_t));
}
 ...
 * @param session a rtp session
 * @param data an opaque pter to be stored in the session
**/
...
/**
 * @param session a rtp session
 * @return the void pter previously set using rtp_session_set_data()
**/
...
static float compute_bw(struct timeval *orig, u32 bytes){
   ...
   time=(float)(current.tv_sec - orig->tv_sec) +
      ((float)(current.tv_usec - orig->tv_usec)*1e-6f);
   bw=((float)bytes)*8/(time+0.001f);
...
void rtp_add_csrc(mblk_t *mp, u32 csrc)
{
...
/**
 * get a pter to the beginning of the payload data of the RTP packet.
 * @param packet a RTP packet represented as a mblk_t
 * @param start a pter to the beginning of the payload data, pting inside the packet.
 * @return the length of the payload data.
**/
i32 rtp_get_payload(mblk_t *packet, uchar **start){
   uchar *tmp;
   i32 header_len=RTP_FIXED_HEADER_SIZE+(rtp_get_cc(packet)*4);
   tmp=packet->b_rptr+header_len;
   if (tmp>packet->b_wptr){
      if (packet->b_cont!=nullptr){
         tmp=packet->b_cont->b_rptr+(header_len- (packet->b_wptr-packet->b_rptr));
         if (tmp<=packet->b_cont->b_wptr){
            *start=tmp;
            return (i32) ( packet->b_cont->b_wptr-tmp);
         }
      }
...
u32 rtp_session_time_to_ts(RtpSession *session, i32 millisecs){
   ...
   payload =
      rtp_profile_get_payload (session->snd.profile,
                session->snd.payloadtype);
   if (payload == nullptr)
   {
      ortp_warning
         ("rtp_session_ts_to_t: use of unsupported payload type %d.", session->snd.payloadtype);
      return 0;
   }
   /* the return value is in milisecond */
   return (u32) (payload->clock_rate*(double) (millisecs/1000.0f));
}
...
void rtp_session_process (RtpSession * session, u32 time, RtpScheduler *sched)
{
   wait_pt_lock(&session->snd.wp);
   if (wait_pt_check(&session->snd.wp,time)){
      session_set_set(&sched->w_sessions,session);
      wait_pt_wakeup(&session->snd.wp);
   }
   wait_pt_unlock(&session->snd.wp);

   wait_pt_lock(&session->rcv.wp);
   if (wait_pt_check(&session->rcv.wp,time)){
      session_set_set(&sched->r_sessions,session);
      wait_pt_wakeup(&session->rcv.wp);
   }
   wait_pt_unlock(&session->rcv.wp);
}
...
namespace rtp
{
   ...
   session::session(i32 mode)
   {
      m_psession = rtp_session_new(mode);
   }


   void session::set_scheduling_mode(i32 yesno)
   {
      rtp_session_set_scheduling_mode(m_psession, yesno);
   }

   void session::set_blocking_mode(i32 yesno)
   {
      rtp_session_set_blocking_mode(m_psession, yesno);
   }

   void session::set_profile(profile * pprofile)
   {
      rtp_session_set_profile(m_psession, pprofile->m_pprofile);
   }

   void session::set_send_profile(profile * pprofile)
   {
      rtp_session_set_send_profile(m_psession, pprofile->m_pprofile);
   }

   void session::set_recv_profile(profile * pprofile)
   {
      rtp_session_set_recv_profile(m_psession, pprofile->m_pprofile);
   }
   ...
   i32 session::set_payload_type(i32 payloadtype)
   {
      return rtp_session_set_payload_type(m_psession, payloadtype);
   }

   i32 session::recv_with_ts(void * buffer, i32 len, u32 ts, i32 *have_more)
   {
      return rtp_session_recv_with_ts(m_psession, (u8 *)buffer, len, ts, have_more);
   }

   i32 session::send_with_ts(const void * buffer, i32 len, u32 userts)
   {
      return rtp_session_send_with_ts(m_psession, (const u8 *) buffer, len, userts);
   }




   void session::init(i32 mode)
   {
      rtp_session_init(m_psession, mode);
   }

   void session::set_flag(i32 flag)
   {
      m_psession->flags |= flag;
   }

   void session::unset_flag(i32 flag)
   {
      m_psession->flags &= ~flag;
   }
