...
void jitter_control_init(JitterControl *ctl, i32 base_jiitt_time, PayloadType * payload){
   ctl->count=0;
   ctl->slide=0;
   ctl->prev_slide=0;
   ctl->jitter=0;
...
void jitter_control_set_payload(JitterControl *ctl, PayloadType *payloadtype){
   ctl->jitt_comp_ts =
         (i32) (((double) ctl->jitt_comp / 1000.0) * (payloadtype->clock_rate));
   ctl->corrective_step=(160 * 8000 )/payloadtype->clock_rate; /* This formula got to me after some beers */
   ctl->adapt_jitt_comp_ts=ctl->jitt_comp_ts;
}
...
void jitter_control_update_corrective_slide(JitterControl *ctl){
   i32 tmp;
   tmp=(i32)((ctl->slide)-ctl->prev_slide);
...
*/
void jitter_control_new_packet(JitterControl *ctl, u32 packet_ts, u32 cur_str_ts){
   i64 diff=(i64)packet_ts - (i64)cur_str_ts;
   double gap,slide;
   i32 d;
   //debug_print("diff=%g\n",diff);
   if (ctl->count==0)
   ...
   if (ctl->adaptive){
      ...
      if (ctl->count%50==0) {
         ctl->adapt_jitt_comp_ts=(i32) max(ctl->jitt_comp_ts,2*ctl->jitter);
 ...
 **/
void
rtp_session_set_jitter_compensation (RtpSession * session, i32 milisec)
{
   PayloadType * payload=nullptr;
   if (session->rcv.payloadtype!=-1) {
...
void rtp_session_get_jitter_buffer_params(RtpSession *session, JBParameters *par){
   i32 nom_size=session->rtp.jittctl.jitt_comp;
