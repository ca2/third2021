...
/**
 * Allocates and initialize a new is_empty session set.
 *
 * @return the is_empty session set.
**/
...
i32 session_set_and(SessionSet *sched_set, i32 maxs, SessionSet *user_set, SessionSet *result_set)
{
   u32 *mask1,*mask2,*mask3;
   i32 i=0;
   i32 j,ret=0;
   mask1=(u32*)(void*)&sched_set->rtpset;
   mask2=(u32*)(void*)&user_set->rtpset;
   mask3=(u32*)(void*)&result_set->rtpset;
   while(i<maxs+1){
   ...
   //debug_print("session_set_and: ret=%i\n",ret);
   return ret;
}
 ...
 *   three sets passed in argument. Two of the sets can be nullptr.
 *   The first set @recvs is interpreted as a set of RtpSession waiting for receive events:
 *   a new buffer (perhaps is_empty) is availlable on one or more sessions of the set, or the last
...
**/
i32 session_set_select(SessionSet *recvs, SessionSet *sends, SessionSet *errors)
{
   i32 ret=0,bits;
   SessionSet temp;
   ...
   while(1){
      ...
      if (recvs!=nullptr){
         ...
         ret+=bits;
         /* copy the result set in the given user set (might be is_empty) */
      ...
      if (ret>0){
         ...
         //debug_print("There are %i sessions set, returning.\n",ret);
         rtp_scheduler_unlock(sched);
         return ret;
      }
      //debug_print("There are %i sessions set.\n",ret);
