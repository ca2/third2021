...
void ortp_exit()
{
   if (__ortp_scheduler!=nullptr)
   {
      rtp_scheduler_destroy(__ortp_scheduler);
      __ortp_scheduler=nullptr;
   }
}

RtpScheduler * ortp_get_scheduler()
{
   if (__ortp_scheduler==nullptr) ortp_error("Cannot use the scheduled mode: the scheduler is not "
...
/**
 *@param file a FILE pter where to output the ortp logs.
 *
**/
...
u32 __ortp_log_mask=ORTP_WARNING|ORTP_ERROR|ORTP_FATAL;
...
#endif
   if ((p = (char *) ortp_malloc (size)) == nullptr)
      return nullptr;
   while (1)
   {
      ...
      if (n > -1 && n < size)
         return p;
      //debug_print("Reallocing space.\n");
      /* Else try again with more space. */
      if (n > -1)   /* glibc 2.1 */
         size = n + 1;   /* precisely what is needed */
      else      /* glibc 2.0 */
         size *= 2;   /* twice the old size */
      if ((np = (char *) ortp_realloc (p, size)) == nullptr)
        {
          free(p);
          return nullptr;
        }
...
void ortp_logv(i32 level, const char *fmt, va_list args)
{
   if (ortp_logv_out!=nullptr && ortp_log_level_enabled(level))
      ortp_logv_out((OrtpLogLevel) level,fmt,args);
...
static void __ortp_logv_out(OrtpLogLevel lev, const char *fmt, va_list args){
   ...
   char *msg;
   if (__log_file==nullptr) __log_file=stderr;
   switch(lev){
      case ORTP_DEBUG:
         lname="debug";
         break;
...
#ifdef ENABLE_MEMCHECK   
   debug_print("Unfreed allocations: %i\n",ortp_allocations);
#endif
}
...
void rtp_stats_display(const rtp_stats_t *stats, const char *header)
{
  ...
  ortp_log(ORTP_MESSAGE,
      ...
      (u64)stats->unavaillable);
  ortp_log(ORTP_MESSAGE,
      " number of rtp packet lost=%I64d",
      (u64) stats->cum_packet_loss);
  ortp_log(ORTP_MESSAGE,
...
void ortp_global_stats_reset(){
   __memset(&ortp_global_stats,0,sizeof(rtp_stats_t));
}
...
void rtp_stats_reset(rtp_stats_t *stats){
   __memset((void*)stats,0,sizeof(rtp_stats_t));
}
