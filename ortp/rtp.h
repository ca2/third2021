...
typedef struct rtp_stats
{
   u64 packet_sent;
   u64 sent;      /* bytes sent */
   u64 recv;       /* bytes of payload received and delivered in time to the application */
   u64 hw_recv;      /* bytes of payload received */
   u64 packet_recv;   /* number of packets received */
   u64 unavaillable;   /* packets not availlable when they were queried */
   u64 outoftime;      /* number of packets that were received too late */
   u64 cum_packet_loss; /* cumulative number of packet lost */
   u64 bad;         /* packets that did not appear to be RTP */
   u64 discarded;      /* incoming packets discarded because the queue exceeds its max size */
} rtp_stats_t;

#define RTP_TIMESTAMP_IS_NEWER_THAN(ts1,ts2) \
   ((u32)((u32)(ts1) - (u32)(ts2))< (u32)(1<<31))

#define RTP_TIMESTAMP_IS_STRICTLY_NEWER_THAN(ts1,ts2) \
   ( ((u32)((u32)(ts1) - (u32)(ts2))< (u32)(1<<31)) && (ts1)!=(ts2) )
...
#define rtp_set_payload_type(mp, point)   ((rtp_header_t*)((mp)->b_rptr))->paytype=(point)
