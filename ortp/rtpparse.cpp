...
static void queue_packet(queue_t *q, i32 maxrqsz, mblk_t *mp, rtp_header_t *rtp, i32 *discarded)
{
   mblk_t *tmp;
   i32 header_size;
   *discarded=0;
...
void rtp_session_rtp_parse(RtpSession *session, mblk_t *mp, u32 local_str_ts, struct sockaddr *addr, socklen_t addrlen)
{
   i32 i;
   rtp_header_t *rtp;
   i32 msgsize;
   ...
   if (rtp->version!=2)
   {
      ...
      u16 stunlen=*((u16*)(mp->b_rptr + sizeof(u16)));
      ...
      if (stunlen+20==mp->b_wptr-mp->b_rptr){
         ...
         if (session->eventqs!=nullptr){
            ...
            ed->packet=mp;
            ed->endpoint=rtp_endpoint_new(addr,addrlen);
   ...
   if (rtp->cc*sizeof(u32) > (u32) (msgsize-RTP_FIXED_HEADER_SIZE)){
      ortp_debug("Receiving too i16 rtp packet.");
      stats->bad++;
      ortp_global_stats.bad++;
      freemsg(mp);
      return;
         ...
         if (session->inc_same_ssrc_count>SSRC_CHANGED_THRESHOLD){
            ...
            if (!session->use_connect){
               if (session->rtp.socket>0 && session->symmetric_rtp){
                  /* store the sender rtp address to do symmetric RTP */
                  ::memcpy_dup(&session->rtp.rem_addr,addr,addrlen);
   ...
   if (session->flags & RTP_SESSION_FIRST_PACKET_DELIVERED) {
      ...
      if (RTP_TIMESTAMP_IS_NEWER_THAN(rtp->timestamp,session->rtp.rcv_last_ts+session->rtp.ts_jump)){
         ...
         rtp_signal_table_emit2(&session->on_timestamp_jump,(uptr)&rtp->timestamp);
      }
