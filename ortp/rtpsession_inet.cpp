...
static bool_t try_connect(i32 fd, const struct sockaddr *dest, socklen_t addrlen){
...
static ortp_socket_t create_and_bind(const char *addr, i32 port, i32 *sock_family, bool_t reuse_addr){
   i32 err;
   i32 optval = 1;
...
static void set_socket_sizes(i32 sock, u32 sndbufsz, u32 rcvbufsz){
   i32 err;
   bool_t done=FALSE;
   if (sndbufsz>0){
...
#endif
      if (!done){
         err = setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (const char *)&sndbufsz, sizeof(sndbufsz));
         if (err == -1) {
...
#endif
      if (!done){
         err = setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (const char *)&rcvbufsz, sizeof(rcvbufsz));
         if (err == -1) {
...
static ortp_socket_t create_and_bind_random(const char *localip, i32 *sock_family, i32 *port){
   i32 retry;
   ortp_socket_t sock = (ortp_socket_t)-1;
...
i32
rtp_session_set_local_addr (RtpSession * session, const char * addr, i32 port)
{
   ortp_socket_t sock;
   i32 sockfamily;
   bool_t reuse_addr;
   ...
   if (sock!=-1){
      set_socket_sizes((i32) sock,session->rtp.snd_socket_size,session->rtp.rcv_socket_size);
...
static char * ortp_inet_ntoa(struct sockaddr *addr, i32 addrlen, char *dest, i32 destlen){
#ifdef ORTP_INET6
   i32 err;
   dest[0]=0;
   err=getnameinfo(addr,addrlen,dest,destlen,nullptr,0,NI_NUMERICHOST);
   if (err!=0){
      ortp_warning("getnameinfo error: %s",gai_strerror(err));
   }
#else
   char *tmp=inet_ntoa(((struct sockaddr_in*)addr)->sin_addr);
   ansi_count_copy(dest,tmp,destlen);
   dest[destlen-1]='\0';
#endif
   return dest;
}
...
**/
i32
rtp_session_set_remote_addr (RtpSession * session, const char * addr, i32 port){
...
rtp_session_set_remote_addr_full (RtpSession * session, const char * addr, i32 rtp_port, i32 rtcp_port)
{
   ...
   for (res = res0; res; res = res->ai_next) {
      ...
      if (res->ai_family==session->rtp.sockfamily ) {
         ::memcpy_dup( &session->rtp.rem_addr, res->ai_addr, res->ai_addrlen);
         session->rtp.rem_addrlen=(i32) res->ai_addrlen;
           err=0;
           break;
      }
   }
   ...
   for (res = res0; res; res = res->ai_next) {
      ...
      if (res->ai_family==session->rtp.sockfamily ) {
           err=0;
           ::memcpy_dup( &session->rtcp.rem_addr, res->ai_addr, res->ai_addrlen);
         session->rtcp.rem_addrlen=(i32) res->ai_addrlen;
           break;
      }
   }
   freeaddrinfo(res0);
   ...
   ::memcpy_dup (&session->rtcp.rem_addr, &session->rtp.rem_addr,
   ...
   if (can_connect(session)){
      if (try_connect((i32) session->rtp.socket,(struct sockaddr*)&session->rtp.rem_addr,session->rtp.rem_addrlen))
         session->flags|=RTP_SOCKET_CONNECTED;
      if (session->rtcp.socket>=0){
         if (try_connect((i32) session->rtcp.socket,(struct sockaddr*)&session->rtcp.rem_addr,session->rtcp.rem_addrlen))
...
i32
rtp_session_set_remote_addr_and_port(RtpSession * session, const char * addr, i32 rtp_port, i32 rtcp_port){
...
void rtp_session_flush_sockets(RtpSession *session){
   ...
   if (rtp_session_using_transport(session, rtp))
     {
      mblk_t *trashmp=esballoc(trash,sizeof(trash),0,nullptr);
...
#ifdef USE_SENDMSG
#define MAX_IOV 30
static i32 rtp_sendmsg(i32 sock,mblk_t *m, struct sockaddr *rem_addr, i32 addr_len){
   i32 error;
   struct msghdr msg;
   struct iovec iov[MAX_IOV];
   i32 iovlen;
   for(iovlen=0; iovlen<MAX_IOV && m!=nullptr; m=m->b_cont,iovlen++){
...
static void update_sent_bytes(RtpSession*s, i32 nbytes){
   if (s->rtp.sent_bytes==0){
      gettimeofday(&s->rtp.send_bw_start,nullptr);
   }
   s->rtp.sent_bytes+=nbytes+IP_UDP_OVERHEAD;
}

static void update_recv_bytes(RtpSession*s, i32 nbytes){
   if (s->rtp.recv_bytes==0){
      gettimeofday(&s->rtp.recv_bw_start,nullptr);
   }
...
rtp_session_rtp_send (RtpSession * session, mblk_t * m)
{
   ...
   if (rtp_session_using_transport(session, rtp)){
      ...
      if (m->b_cont!=nullptr)
         msgpullup(m,-1);
      error = (i32) sendto (sockfd, (char*)m->b_rptr, (i32) (m->b_wptr - m->b_rptr),
          0,destaddr,destlen);
#endif
   }
   if (error < 0){
      if (session->on_network_error.count>0){
         rtp_signal_table_emit3(&session->on_network_error,(uptr)"Error sending RTP packet",INT_TO_POINTER(getSocketErrorCode()));
...
rtp_session_rtcp_send (RtpSession * session, mblk_t * m)
{
   ...
   if (session->rtcp.enabled &&
      ...
      if (rtp_session_using_transport(session, rtcp)){
         ...
         error = (i32) sendto (sockfd, (char*)m->b_rptr,
         (i32) (m->b_wptr - m->b_rptr), 0,
         destaddr, destlen);
#endif
      }
      if (error < 0){
         char host[65];
         if (session->on_network_error.count>0){
            rtp_signal_table_emit3(&session->on_network_error,(uptr)"Error sending RTCP packet",INT_TO_POINTER(getSocketErrorCode()));
...
i32
rtp_session_rtp_recv (RtpSession * session, u32 user_ts)
{
   i32 error;
   ortp_socket_t sockfd=session->rtp.socket;
#ifdef ORTP_INET6
      ...
      if (error > 0){
         if (session->symmetric_rtp && !sock_connected){
            if (session->use_connect){
               ...
               ::memcpy_dup(&session->rtp.rem_addr,&remaddr,addrlen);
               session->rtp.rem_addrlen=addrlen;
               if (try_connect((i32) sockfd,(struct sockaddr*)&remaddr,addrlen))
                  session->flags|=RTP_SOCKET_CONNECTED;
            }
         }
         ...
         session->rtp.cached_mp=nullptr;
         /*for bandwidth measurements:*/
         update_recv_bytes(session,error);
      }
      else
      {
          i32 errnum=getSocketErrorCode();
         ...
         {
            if (session->on_network_error.count>0){
               rtp_signal_table_emit3(&session->on_network_error,(uptr)"Error receiving RTP packet",INT_TO_POINTER(getSocketErrorCode()));
...
rtp_session_rtcp_recv (RtpSession * session)
{
   ...
   while (1)
   {
      ...
      if (session->rtcp.cached_mp==nullptr)
          session->rtcp.cached_mp = allocb (RTCP_MAX_RECV_BUFSIZE, 0);

      mp=session->rtcp.cached_mp;
      if (sock_connected){
         error=(i32) recv(session->rtcp.socket,(char*)mp->b_wptr,RTCP_MAX_RECV_BUFSIZE,0);
      }else {
         ...
         if (rtp_session_using_transport(session, rtcp))
           ...
           error=(i32) recvfrom (session->rtcp.socket,(char*) mp->b_wptr,
      ...
      if (error > 0)
      {
         ...
         session->rtcp.cached_mp=nullptr;
         if (session->symmetric_rtp && !sock_connected){
            /* store the sender rtp address to do symmetric RTP */
            ::memcpy_dup(&session->rtcp.rem_addr,&remaddr,addrlen);
            session->rtcp.rem_addrlen=addrlen;
            if (session->use_connect){
               if (try_connect((i32) session->rtcp.socket,(struct sockaddr*)&remaddr,addrlen))
                  session->flags|=RTCP_SOCKET_CONNECTED;
            }
         }
      }
      else
      {
         ...
         {
            if (session->on_network_error.count>0){
               rtp_signal_table_emit3(&session->on_network_error,(uptr)"Error receiving RTCP packet",INT_TO_POINTER(errnum));
