...
__declspec(dllexport) HANDLE CreateFileW(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
...
__declspec(dllexport) HANDLE CreateFileA(LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
                   LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition,
                   DWORD dwFlagsAndAttributes, HANDLE hTemplateFile)
{
	HANDLE hFile;
...
__declspec(dllexport) DWORD GetFileSize(HANDLE hFile, LPDWORD lpFileSizeHigh)
{
	BOOL status;
...
__declspec(dllexport) DWORD GetLogicalDriveStringsA(DWORD nBufferLength, LPSTR lpBuffer)
{
