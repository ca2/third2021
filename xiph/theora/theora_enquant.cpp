  ...
  for(qti=0;qti<2;qti++)for(pli=0;pli<3;pli++){
    ...
    for(qri=0;qri<=qranges->nranges;qri++){
      for(bmi=0;;bmi++){
        ...
        else if(memcmp_dup(base_mats[bmi][0],qranges->base_matrices[qri],
    ...
    if(i>0){
      if(qti>0){
        if(qranges->nranges==_qinfo->qi_ranges[qti-1][pli].nranges&&
         memcmp_dup(qranges->sizes,_qinfo->qi_ranges[qti-1][pli].sizes,
         qranges->nranges*sizeof(qranges->sizes[0]))==0&&
         memcmp_dup(indices[qti][pli],indices[qti-1][pli],
      ...
      if(qranges->nranges==_qinfo->qi_ranges[qtj][plj].nranges&&
       memcmp_dup(qranges->sizes,_qinfo->qi_ranges[qtj][plj].sizes,
       qranges->nranges*sizeof(qranges->sizes[0]))==0&&
       memcmp_dup(indices[qti][pli],indices[qtj][plj],
       (qranges->nranges+1)*sizeof(indices[qti][pli][0]))==0){
