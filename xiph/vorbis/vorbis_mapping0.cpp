...
static int32_t mapping0_forward(vorbis_block *vb){
  ...
  for(i=0;i<vi->channels;i++){
    ...
    logfft[0]=(float) (scale_dB+todB(pcm)  + .345);
                                  /* + .345 is a hack; the
...

  {
    ...
    for(i=0;i<vi->channels;i++){
      ...
      for(j=0;j<n/2;j++)
        logmdct[j]= (float) (todB(mdct+j)  + .345); 
                                  /* + .345 is a hack; the original
                                     todB estimation used on IEEE 754
                                     compliant machines had a bug that
                                     returned dB values about a third
    ...
    for(k=(vorbis_bitrate_managed(vb)?0:PACKETBLOBS/2);
      ...
      for(i=0;i<info->submaps;i++){
        ...
        classifications=_residue_P[ci->residue_type[resnum]]->
          m_class(vb,b->residue[resnum],couple_bundle,zerobundle,ch_in_bundle);
...
/* export hooks */
extern "C" const vorbis_func_mapping mapping0_exportbundle={
  &mapping0_pack,
  &mapping0_unpack,
  &mapping0_free_info,
  &mapping0_forward,
