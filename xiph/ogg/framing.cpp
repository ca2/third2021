...
long ogg_sync_pageseek(ogg_sync_state *oy,ogg_page *og){
  ...
  if(oy->headerbytes==0){
    ...
    if(memcmp_dup(page,"OggS",4))goto sync_fail;
  ...
  /* The whole test page is buffered.  Verify the checksum */
  {
    ...
    /* Compare */
    if(memcmp_dup(chksum,page+22,4)){
...

        {
          ...
          while(1){
            ...
            while(ogg_stream_packetpeek(&os_de,&op_de2)>0){
              ...
              if(memcmp_dup(&op_de,&op_de2,sizeof(op_de))){
              ...
              /* verify the packet! */
              /* check data */
              if(memcmp_dup(data+depacket,op_de.packet,op_de.bytes)){
